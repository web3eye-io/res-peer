use review::{InitialState, RewardType};
use linera_sdk::{
...
pub struct Review {
    pub review_balance: RegisterView<Amount>,
...
impl Review {
    ...
    pub(crate) async fn deposit(&mut self, amount: Amount) -> Result<(), StateError> {
        ...
        let _amount = self.review_balance.get().try_add(_amount)?;

        self.review_balance.set(_amount);
        Ok(())
    }
